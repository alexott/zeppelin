...
object ParagraphParser {
  ...
  val GENERIC_STATEMENT_PREFIX =
    """(?is)\s*(?:INSERT|UPDATE|DELETE|SELECT|CREATE|ALTER|
      |DROP|GRANT|REVOKE|TRUNCATE|LIST|USE|RELOAD|REBUILD|COMMIT|RESTRICT|UNRESTRICT)\s+""".r
  ...
  def extractSearchIndexCmd(text: String): DescribeSearchIndexCmd = {
    text match {
      ...
      case DESCRIBE_SEARCH_INDEX_PATTERN(status, what, view) =>
        throw new InterpreterException("DESCRIBE SEARCH INDEX works only with full qualified table name: please specify keyspace!")
